 ...
 *
 * @see {@link http://www.info.ufrn.br/wikisistemas/doku.php?id=suporte:iproject:visao_geral}
 *
 ...
 *
 */
public class IprojectMiner extends Miner{
	...
	public ChangeLogHistory performMining(Properties configurationKnowledge) throws Exception{
		...
		System.out.println(" Reading information of Iproject 1 of 3... ");
		
		IprojectConnector iprojectConnector = ( (IprojectConnector) systemConnector);
		iprojectConnector.performsRun();
		IprojectBuildInformation iprojectBuildLogs = iprojectConnector.getIprojectBuildInformation();
		
		if(iprojectBuildLogs == null)
			return null;
		
		System.out.println(" Reading information of Iproject in "+((System.currentTimeMillis()-startTime)/1000/60)+" minutes ");
		
		System.out.println(" Converting Iprojet Tasks To ChangeLogHistory Model 2 of 3 ...  ");
		ChangeLogHistory changeLogHistory = convertIprojetTaskToChangeLogHistoryModel(iprojectBuildLogs, configurationKnowledge);
		System.out.println(" Converting Iprojet Tasks To ChangeLogHistory Model in "+((System.currentTimeMillis()-startTime)/1000/60)+" minutes ");
	...
	/** Convert the initial information retrieved by IPROJECT to the ChangeLogHistoryModel */
	private ChangeLogHistory convertIprojetTaskToChangeLogHistoryModel(IprojectBuildInformation iprojectBuildInformation, Properties configurationKnowledge){
		...
		changeLogHistory.setSystem(iprojectBuildInformation.getSystem() );
		
		nextTask:
		for (IprojectTaskInformation iprojectTask : iprojectBuildInformation.getTasks()) {
			
			ChangeLog changeLog = new ChangeLog();
			
			changeLog.setIdentify(iprojectTask.getTaskNumber());
			changeLog.setDescription( StringUtils.toAscii( iprojectTask.getTaskName() ) );
			changeLog.setModule(  StringUtils.toAscii( iprojectTask.getModule() ) );
			changeLog.getRevisionRelease().addAll( iprojectTask.getRevisionRelease() );
			
			////  The  type of ChangeLog  ////
			if(iprojectTask.getTaskType().contains("ERRO"))
				changeLog.setType(ChangeLogType.BUG_FIX);
			else if(iprojectTask.getTaskType().contains("APRIMORAMENTO") || iprojectTask.getTaskType().contains("ALTERA��O") )
			...
			for (String chageLogs : iprojectTask.getChangeLogs()){
				if(changeLog.getChangeInformation() == null)
					...
					if(! changeLog.getChangeInformation().replaceAll("\\s", "").replace("ChangeLog:", "").replaceAll("<br>", "")
							.contains( chageLogs.replaceAll("\\s", "").replace("ChangeLog:", "").replaceAll("<br>", "") ) ) // eliminates duplicate CHANGELOG
			...
			for (String log : iprojectTask.getLogs()){
				changeLog.addClassesChangeLog( IprojectLogUtils.extractClassesFromLog(log,  systemConnector.getSystemName() ) );
			}
			
			if (changeLog.getClasses().size() == 0) // some task of IPROJECT doesn't have revision
				continue nextTask;
			
			// associate with the respective feature, if is not associated, put into the CORE feature
			
			List<FeatureChangeLog> featuresMapping = ConfigurationKnowledgeUtil.getFeaturesOfATask(configurationKnowledge, iprojectTask.getTaskNumber());
